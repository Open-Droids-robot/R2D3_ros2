...
def generate_launch_description():
    ...
    declare_world = DeclareLaunchArgument(
        'world',
        default_value=os.path.join(pkg_nav, 'worlds', 'nav_empty.sdf'),
    ...
    urdf_xacro_path = PathJoinSubstitution([
        FindPackageShare('dual_rm_navigation'), 'urdf',
        PythonExpression([
            "'r2d3_' + '", robot_model, "' + '_nav_sim.urdf.xacro'"
        ]),
    ])
    ...
    set_resource_path_worlds = AppendEnvironmentVariable(
        'GZ_SIM_RESOURCE_PATH', os.path.join(pkg_nav, 'worlds'))
    ...
    bridge = Node(
        ...
        name='ros_gz_bridge',
        parameters=[{
            'config_file': os.path.join(pkg_nav, 'config', 'gz_bridge.yaml'),
            'use_sim_time': True,
        }],
        output='screen',
    )
    ...
    # ── RViz2 (optional) 
    rviz_config = os.path.join(pkg_nav, 'rviz', 'nav2_view.rviz')
    rviz_node = Node(
